export default async function handler(req, res) {
  ...
  try {
    ...
    const receipt = `<h2>Royal Burger Ibadan — ${orderId}</h2><p>Hi ${order.customer.name}, thanks for ordering!</p><ul>${order.items.map(i => `<li>${i.qty} × ${i.name} — ₦${Number(i.price * i.qty).toLocaleString()}</li>`).join('')}</ul><h3>Total: ₦${Number(order.total).toLocaleString()}</h3><p>WhatsApp: ${order.customer.phone}<br>Address: ${order.customer.address}</p>`;
    ...
    if (process.env.RESEND_API_KEY && process.env.RECEIPT_FROM_EMAIL) {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `Royal Burger receipt ${orderId}`,
          html: receipt
        })
      });
      emailSent = r.ok;
    ...
    if (process.env.WHATSAPP_ACCESS_TOKEN && process.env.WHATSAPP_PHONE_NUMBER_ID) {
      ...
      const r = await fetch(`https://graph.facebook.com/v23.0/${process.env.WHATSAPP_PHONE_NUMBER_ID}/messages`, {
        ...
        body: JSON.stringify({
          ...
          type: 'text',
          text: {
            body: `Royal Burger receipt ${orderId}\n${order.items.map(i => `${i.qty}x ${i.name}`).join('\n')}\nTotal: ₦${Number(order.total).toLocaleString()}\nAddress: ${order.customer.address}`
          }
        })
      });
      whatsappSent = r.ok;
